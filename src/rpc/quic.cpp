...
namespace {
    const command_line::arg_descriptor<std::vector<std::string>> arg_quic{
            "quic",
            "Adds a QUIC listener on the given IP address and UDP port.  Can be specified multiple "
            "times.  Note that when running as a service node, a QUIC listener will also be "
            "running on the service node's public IP without needing to pass this argument.  "
            "Examples: :5555 (listen on UDP port 5555, all addresses), 0.0.0.0:5555 (same as "
            ":5555), 198.51.100.42:5555 (port 5555 on specific IPv4 address); or an IPv6 address "
            "such as [::]:5555 or [2001:db8::abc]:5555 (IPv6)."};
    const command_line::arg_descriptor<bool> arg_quic_public_rpc{
            "quic-public",
            "Allow unathenticated public RPC requests on the node's listening QUIC address(es).  "
            "If this option is not provided then any incoming connection that is not identifiable "
            "as coming from a service node or a configured --quic-user or --quic-admin pubkey will "
            "be immediately dropped."};
    const command_line::arg_descriptor<std::vector<std::string>> arg_quic_user{
            "quic-user",
            "Specify an Ed25519 pubkey to allow non-admin RPC commands from a client "
            "authenticating with that pubkey.  Can be specified multiple times."};
    const command_line::arg_descriptor<std::vector<std::string>> arg_quic_admin{
            "quic-admin",
            "Specify an Ed25519 pubkey to allow unrestricted, admin-level RPC commands from a "
            "client authenticating with that pubkey.  Can be specified multiple times.  When "
            "running as a service node the server's Ed25519 pubkey will automatically have this "
            "permission."};

    // QUIC RPC request responses are tagged (at the protocol level) as either success or an error:
    // for success we simply send the successful body.  For errors we sent:
    //
    //     NNN Status
    //
    //     body
    //
    // where NNN is an HTTP-like numeric code and Status is a status phrase; for example the first
    // line could be: "403 Forbidden".  This is followed by an empty line, and then the respond body
    // (if any).
    constexpr std::string_view QUIC_NOT_FOUND{"404 Not Found"sv}, QUIC_FORBIDDEN{"403 Forbidden"sv};

}  // namespace

void init_quic_options(boost::program_options::options_description& desc) {
    command_line::add_arg(desc, arg_quic);
    command_line::add_arg(desc, arg_quic_public_rpc);
    command_line::add_arg(desc, arg_quic_user);
    command_line::add_arg(desc, arg_quic_admin);
}
...
namespace cryptonote { namespace rpc {
    ...
    omq_rpc::omq_rpc(
        ...
        for (auto& cmd : rpc_commands) {
            omq.add_request_command(
                    ...
                    [name = std::string_view{cmd.first}, &call = *cmd.second, this](
                        ...
                        m.send_reply(
                                LMQ_ERROR, "An exception occured while processing your request");
                    });
        }
        ...
        // indicates that you were already subscribed for the request tx types.  Any other value
        ...
        // replies as a indicator that there was some server-side interruption (such as a restart)
        // that might necessitate the client rechecking the mempool.
        //
        // When a tx arrives the node sends back [notify.mempool, txhash, txblob] every time a new
        // transaction is added to the mempool (minus some additions that aren't really new
        // transactions such as txes that came from an existing block during a rollback).  Note that
        // both txhash and txblob are binary: in particular, txhash is *not* hex-encoded.
        //
...

            {
                ...
                if (!result.second) {
                    ...
                    if (result.first->second.type == sub_type) {
                        MTRACE("Renewed mempool subscription request from conn id "
                               << m.conn << " @ " << m.remote);
                ...
                MDEBUG("New " << (sub_type == mempool_sub_type::blink ? "blink" : "all")
                              << " mempool subscription request from conn " << m.conn << " @ "
                              << m.remote);
                m.send_reply("OK");
            }
        });

        // New block subscriptions: [sub.block].  This sends a notification every time a new block
        // is added to the blockchain.
        //
        // TODO: make this support [sub.block, sn] so that we can receive notification only for
        // blocks that change the SN composition.
        //
        // The subscription request returns the current [height, blockhash] as a reply.
        //
        // The block notification for new blocks consists of a message [notify.block, height,
        // blockhash] containing the latest height/hash.  (Note that blockhash is the hash in bytes,
        // *not* the hex encoded block hash).
        omq.add_request_command("sub", "block", [this](oxenmq::Message& m) {
            ...
            if (!result.second) {
                result.first->second.expiry = expiry;
                MTRACE("Renewed block subscription request from conn id " << m.conn << " @ "
                                                                          << m.remote);
                m.send_reply("ALREADY");
            } else {
    ...
    static void send_notifies(Mutex& mutex, Subs& subs, const char* desc, Call call) {
        ...
        for (auto& conn : remove) {
            ...
            if (it != subs.end() &&
                ...
                MDEBUG("Removing " << conn << " from " << desc
                                   << " subscriptions: subscription timed out");
                subs.erase(it);
            }
        }
    }
    ...
    void omq_rpc::send_block_notifications(const block& block) {
        ...
        send_notifies(subs_mutex_, block_subs_, "block", [&](auto& conn, auto& sub) {
            omq.send(
                    ...
                    "notify.block",
                    height,
                    std::string_view{block.hash.data, sizeof(block.hash.data)});
        });
    }
